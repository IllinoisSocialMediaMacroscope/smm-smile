...
router.post('/list-all', function (req, res, next) {
    ...
    routesFiles.forEach(function (route, i) {
        if (route.split(".")[0] !== "" && route.split(".")[1] === "json" && fs.lstatSync(path.join(routesDir, route)).isFile()) {
            ...
            if ('result_path' in routesConfig) {
                ...
                order.push({order: j + 10, parent: parent, child: child});
                promise_array.push(list_folders(s3FolderName + routesConfig['result_path']));
    ...
    Promise.all(promise_array).then(values => {
        ...
        directory['GraphQL']['crimson-Hexagon'] = values[8];
